 ...
 * @copyright  2015 E-Learn Design https://www.e-learndesign.co.uk
...
function mycourses_get_my_completion($sort = 'coursefullname', $dir = 'ASC') {
    ...
    $enrolled_courseids = array_column($courses, 'id');
    $sql_course_in = empty($enrolled_courseids) ? -1 : implode(',', $enrolled_courseids);
    ...
    if (!empty($companyid)) {
        $companyselfenrolcourses = $DB->get_records_sql("SELECT e.id,e.courseid,c.fullname as coursefullname, c.summary as coursesummary
                                                         ...
                                                         AND c.visible = 1 AND c.id IN ($sql_course_in)
                                                         $inprogresssql",
