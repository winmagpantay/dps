...
:   Weight of this server in this server group.
    [https://php.net/manual/en/function.Memcache-addServer.php](https://php.net/manual/en/function.Memcache-addServer.php)
...
SimpleSAMLphp uses [PDO](https://www.php.net/manual/en/book.pdo.php) when accessing the database server, so the database source is configured as with a DSN.
The DSN is stored in the `store.sql.dsn` option. See the [PDO driver manual](https://www.php.net/manual/en/pdo.drivers.php) for the DSN syntax used by the different databases.
...
[language codes as specified in ISO-639-1](https://en.wikipedia.org/wiki/List_of_ISO_639-1_codes).
