...
class SimpleTest extends ClearStateTestCase
{
    ...
    public function testGetProcessedURL(): void
    {
        ...
        $this->assertEquals(
            ...
            $method->invokeArgs(
                $s,
                ['https://some.overridden.host/foo/bar?a=b#fragment']
            )
        );
        ...
        Configuration::loadFromArray([
            'application' => [
                'baseURL' => 'https://example.org:8080/'
            ]
        ], '[ARRAY]', 'simplesaml');
        $s = new Auth\Simple('');
        $this->assertEquals('https://example.org:8080/foo/bar?a=b#fragment', $method->invokeArgs($s, [null]));

        // test again with a relative URL as a parameter
        $this->assertEquals(
            'https://example.org:8080/something?foo=bar#something',
