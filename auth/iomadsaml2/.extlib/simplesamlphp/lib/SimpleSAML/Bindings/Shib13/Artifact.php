...
class Artifact
{
    ...
    {
        $msg = '<SOAP-ENV:Envelope xmlns:SOAP-ENV="https://schemas.xmlsoap.org/soap/envelope/">'.
            '<SOAP-ENV:Body>'.
    ...
    private static function extractResponse(string $soapResponse): string
    {
        ...
        if (!Utils\XML::isDOMNodeOfType($soapEnvelope, 'Envelope', 'https://schemas.xmlsoap.org/soap/envelope/')) {
            throw new Error\Exception('Expected artifact response to contain a <soap:Envelope> element.');
        }

        $soapBody = Utils\XML::getDOMChildren($soapEnvelope, 'Body', 'https://schemas.xmlsoap.org/soap/envelope/');
        if (count($soapBody) === 0) {
    ...
    public static function receive(Configuration $spMetadata, Configuration $idpMetadata)
    {
        ...
        $opts = [
            ...
            'http' => [
                'method' => 'POST',
                'content' => $request,
                'header' => 'SOAPAction: https://www.oasis-open.org/committees/security'."\r\n".
