...
class url_finder {
    ...
    protected function domain_swap($table, $column, $domain, $search) {
        ...
        if (isset($renames[$domain])) {
            $replace = preg_replace('|https://'.preg_quote($domain).'|i', 'https://' . $renames[$domain], $search);
        } else {
            $replace = preg_replace('|https://|i', 'https://', $search);
        }
        $DB->set_debug(true);
                ...
                foreach ($columns as $column) {
                    ...
                    if ($column->meta_type == 'X' || ($column->meta_type == 'C' && $column->max_length > 255)) {
                        ...
                        foreach ($rs as $record) {
                            // Regex to match src=https://etc. and data=https://etc.urls.
                            // Standard warning on expecting regex to perfectly parse HTML
                            // read https://stackoverflow.com/a/1732454 for more info.
                            $regex = '#((src|data)\ *=\ *[\'\"])(https://)([^\s()<>]+(?:\([\w\d]+\)|([^[:punct:]\s]|/)))[\'\"]#i';
