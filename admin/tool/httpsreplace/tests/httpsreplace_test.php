 ...
 * @copyright Copyright (c) 2016 Blackboard Inc. (https://www.blackboard.com)
...
class httpsreplace_test extends \advanced_testcase {
    ...
    public function upgrade_http_links_provider() {
        ...
        $wwwroothttp = preg_replace('/^https:/', 'https:', $CFG->wwwroot);
        return [
            ...
            "Test image from a site with international name should be replaced" => [
                "content" => '<img src="https://中国互联网络信息中心.中国/logosy/201706/W01.png">',
                "outputregex" => '/UPDATE/',
            ...
            "Unavailable image should be replaced" => [
                "content" => '<img src="https://intentionally.unavailable/link1.jpg">',
                "outputregex" => '/UPDATE/',
            ...
            "Https content that has an http url as a param should not be replaced" => [
                "content" => '<img src="https://anothersite.com?param=https://asdf.com">',
                "outputregex" => '/^$/',
                "expectedcontent" => '<img src="https://anothersite.com?param=https://asdf.com">',
            ],
    ...
    protected function get_converted_http_link($path) {
        return preg_replace('/^https:/', 'https:', $this->getExternalTestFileUrl($path, false));
    }
    ...
    public function http_link_stats_provider() {
        ...
        return [
            ...
            "Unavailable object should be reported" => [
                "content" => '<object data="https://intentionally.unavailable/file.swf">',
                "domain" => 'intentionally.unavailable',
                "expectedcount" => 1,
            ],
            "Link should not be reported" => [
                "content" => '<a href="https://intentionally.unavailable/page.php">Link</a>',
                "domain" => 'intentionally.unavailable',
                "expectedcount" => 0,
            ],
            "Text should not be reported" => [
                "content" => 'https://intentionally.unavailable/page.php',
                "domain" => 'intentionally.unavailable',
                "expectedcount" => 0,
            ],
        ];
    ...
    public function test_links_and_text() {
        ...
        $generator = $this->getDataGenerator();
        $course = $generator->create_course((object) [
            'summary' => '<a href="https://intentionally.unavailable/page.php">Link</a> https://other.unavailable/page.php',
        ]);
        ...
        $summary = $DB->get_field('course', 'summary', ['id' => $course->id]);
        $this->assertStringContainsString('https://intentionally.unavailable/page.php', $summary);
        $this->assertStringContainsString('https://other.unavailable/page.php', $summary);
    ...
    public function test_httpwwwroot() {
        ...
        $this->resetAfterTest();
        $CFG->wwwroot = preg_replace('/^https:/', 'https:', $CFG->wwwroot);
        $this->expectOutputRegex('/^$/');
    ...
    public function test_upgrade_http_links_excluded_tables() {
        ...
        set_config('test_upgrade_http_links', '<img src="https://somesite/someimage.png" />');
        ...
        $this->assertStringContainsString('https://somesite', $testconf);
    ...
    public function test_renames() {
        ...
        $generator = $this->getDataGenerator();
        $course = $generator->create_course((object) [
            'summary' => '<script src="https://example.com/test.js"><img src="https://EXAMPLE.COM/someimage.png">',
        ]);
        ...
        $this->assertStringNotContainsString('https://example.com', $summary);
    ...
    public function test_multiple() {
        ...
        $expected2 = '';
        for ($i = 0; $i < 15; $i++) {
            $original1 .= '<img src="https://example.com/image' . $i . '.png">';
            $expected1 .= '<img src="https://example.com/image' . $i . '.png">';
            $original2 .= '<img src="https://example.com/image' . ($i + 15 ) . '.png">';
