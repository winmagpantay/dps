...
class brickfield_accessibility_test {
    ...
    public function get_path($file): string {
        if ((substr($file, 0, 7) == 'https://') || (substr($file, 0, 8) == 'https://')) {
            return $file;
        }
        $file = explode('/', $file);
        if (count($file) == 1) {
    ...
    /**
     * Returns if a GIF files is animated or not https://us.php.net/manual/en/function.imagecreatefromgif.php#88005
