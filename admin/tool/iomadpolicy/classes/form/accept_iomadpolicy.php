...
/**
 * Provides {@link tool_iomadpolicy\form\accept_iomadpolicy} class.
 *
...
/**
 * Represents the form for accepting or revoking a iomadpolicy.
 *
 * @package     tool_iomadpolicy
 * @copyright   2018 Marina Glancy
 * @license     https://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class accept_iomadpolicy extends \moodleform {
    ...
    public function definition() {
        ...
        $useracceptancelabel = (count($usernames) > 1) ? get_string('acceptanceusers', 'tool_iomadpolicy') :
                get_string('user');
        $mform->addElement('static', 'user', $useracceptancelabel, join(', ', $usernames));
        $iomadpolicyacceptancelabel = (count($versionnames) > 1) ? get_string('acceptancepolicies', 'tool_iomadpolicy') :
                get_string('iomadpolicydochdriomadpolicy', 'tool_iomadpolicy');
        $mform->addElement('static', 'iomadpolicy', $iomadpolicyacceptancelabel, join(', ', $versionnames));

        if ($action === 'revoke') {
            $mform->addElement('static', 'ack', '', get_string('revokeacknowledgement', 'tool_iomadpolicy'));
            $mform->addElement('hidden', 'action', 'revoke');
        } else if ($action === 'accept') {
            $mform->addElement('static', 'ack', '', get_string('acceptanceacknowledgement', 'tool_iomadpolicy'));
        ...
        $mform->addElement('textarea', 'note', get_string('acceptancenote', 'tool_iomadpolicy'));
        $mform->setType('note', PARAM_NOTAGS);

        if (!empty($this->_customdata['showbuttons'])) {
            if ($action === 'revoke') {
                $this->add_action_buttons(true, get_string('irevoketheiomadpolicy', 'tool_iomadpolicy'));
            } else if ($action === 'accept') {
                $this->add_action_buttons(true, get_string('iagreetotheiomadpolicy', 'tool_iomadpolicy'));
            } else if ($action === 'decline') {
                $this->add_action_buttons(true, get_string('declinetheiomadpolicy', 'tool_iomadpolicy'));
            }
        }

        $PAGE->requires->js_call_amd('tool_iomadpolicy/iomadpolicyactions', 'init', ['[data-action="view"]']);
    }

    /**
     * Validate userids and return usernames
     *
     * @param array $versionids int[] List of iomadpolicy version ids to process.
     * @param array $userids
    ...
    protected function validate_and_get_versions($versionids) {
        ...
        foreach ($versionids as $versionid) {
            ...
            if ($version->status != iomadpolicy_version::STATUS_ACTIVE) {
                $iomadpolicyname .= ' ' . $version->revision;
            }
            $versionnames[$version->id] = \html_writer::link($url, $iomadpolicyname,
            ...
            foreach ($data->userids as $userid) {
                if ($data->action === 'revoke') {
                    foreach ($data->versionids as $versionid) {
                        \tool_iomadpolicy\api::revoke_acceptance($versionid, $userid, $data->note);
                    }
                } else if ($data->action === 'accept') {
                    \tool_iomadpolicy\api::accept_policies($data->versionids, $userid, $data->note);
                } else if ($data->action === 'decline') {
                    \tool_iomadpolicy\api::decline_policies($data->versionids, $userid, $data->note);
                }
            }
        }
    }
