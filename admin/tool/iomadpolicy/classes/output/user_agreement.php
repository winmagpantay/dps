...
/**
 * Provides {@link tool_iomadpolicy\output\user_agreement} class.
 *
...
class user_agreement implements \templatable, \renderable {
     ...
     * @param bool $canaccept does the current user have permission to accept/decline the iomadpolicy on behalf of user $userid
     * @param bool $canrevoke does the current user have permission to revoke the iomadpolicy on behalf of user $userid
     */
    public function __construct($userid, array $accepted, array $declined, moodle_url $pageurl, $versions, $onbehalf = false,
        ...
        if (count($this->accepted) < count($this->versions) && $canaccept === null) {
            $this->canaccept = \tool_iomadpolicy\api::can_accept_policies(array_keys($this->versions), $this->userid);
        }

        if (count($this->accepted) > 0 && $canrevoke === null) {
            $this->canrevoke = \tool_iomadpolicy\api::can_revoke_policies(array_keys($this->versions), $this->userid);
        }
    }
    ...
    public function export_for_template(\renderer_base $output) {
        ...
        if (count($this->versions) == 1) {
            // We represent one particular iomadpolicy's agreement status.
            $versionname = reset($this->versions);
            $versionid = key($this->versions);

            $actionaccept = (object)[
                'text' => get_string('useracceptanceactionaccept', 'tool_iomadpolicy'),
                'title' => get_string('useracceptanceactionacceptone', 'tool_iomadpolicy', $versionname),
                'data' => 'acceptmodal',
                'url' => (new \moodle_url('/admin/tool/iomadpolicy/accept.php', [
                    'userids[]' => $this->userid,
            ...
            $actionrevoke = (object)[
                'text' => get_string('useracceptanceactionrevoke', 'tool_iomadpolicy'),
                'title' => get_string('useracceptanceactionrevokeone', 'tool_iomadpolicy', $versionname),
                'data' => 'acceptmodal',
                'url' => (new \moodle_url('/admin/tool/iomadpolicy/accept.php', [
                    'userids[]' => $this->userid,
                    'versionids[]' => $versionid,
            ...
            $actiondecline = (object)[
                'text' => get_string('useracceptanceactiondecline', 'tool_iomadpolicy'),
                'title' => get_string('useracceptanceactiondeclineone', 'tool_iomadpolicy', $versionname),
                'data' => 'acceptmodal',
                'url' => (new \moodle_url('/admin/tool/iomadpolicy/accept.php', [
                    'userids[]' => $this->userid,
                    'versionids[]' => $versionid,
            ...
            if ($this->accepted) {
                ...
                if ($this->onbehalf) {
                    $data->statustext = get_string('acceptancestatusacceptedbehalf', 'tool_iomadpolicy');
                } else {
                    $data->statustext = get_string('acceptancestatusaccepted', 'tool_iomadpolicy');
                }
                ...
                if ($this->onbehalf) {
                    $data->statustext = get_string('acceptancestatusdeclinedbehalf', 'tool_iomadpolicy');
                } else {
                    $data->statustext = get_string('acceptancestatusdeclined', 'tool_iomadpolicy');
                }
                ...
                $data->statustext = get_string('acceptancestatuspending', 'tool_iomadpolicy');
            ...
            $data->actions[] = (object)[
                'text' => get_string('useracceptanceactiondetails', 'tool_iomadpolicy'),
                'url' => (new \moodle_url('/admin/tool/iomadpolicy/user.php', [
                    'userid' => $this->userid,
                    'returnurl' => $this->pageurl->out_as_local_url(false),
                ]))->out(false),
            ];
            ...
            $accepturl = new \moodle_url('/admin/tool/iomadpolicy/accept.php', [
                'userids[]' => $this->userid,
                'action' => 'accept',
                'returnurl' => $this->pageurl->out_as_local_url(false),
            ]);
            ...
            $actionaccept = (object)[
                'text' => get_string('useracceptanceactionaccept', 'tool_iomadpolicy'),
                'title' => get_string('useracceptanceactionacceptpending', 'tool_iomadpolicy'),
                'data' => 'acceptmodal',
                'url' => $accepturl->out(false),
            ];

            // Prepare the action link to revoke all agreed policies.
            $revokeurl = new \moodle_url('/admin/tool/iomadpolicy/accept.php', [
                'userids[]' => $this->userid,
                'action' => 'revoke',
                'returnurl' => $this->pageurl->out_as_local_url(false),
            ]);
            ...
            $actionrevoke = (object)[
                'text' => get_string('useracceptanceactionrevoke', 'tool_iomadpolicy'),
                'title' => get_string('useracceptanceactionrevokeall', 'tool_iomadpolicy'),
                'data' => 'acceptmodal',
                'url' => $revokeurl->out(false),
            ];

            // Prepare the action link to decline all pending policies.
            $declineurl = new \moodle_url('/admin/tool/iomadpolicy/accept.php', [
                'userids[]' => $this->userid,
                'action' => 'decline',
                'returnurl' => $this->pageurl->out_as_local_url(false),
            ]);
            ...
            $actiondecline = (object)[
                'text' => get_string('useracceptanceactiondecline', 'tool_iomadpolicy'),
                'title' => get_string('useracceptanceactiondeclinepending', 'tool_iomadpolicy'),
                'data' => 'acceptmodal',
            ...
            if ($countaccepted == $countversions) {
                ...
                $data->statustext = get_string('acceptancestatuspartial', 'tool_iomadpolicy');
    ...
    public function export_for_download() {
        ...
        if (count($this->versions) == 1) {
            if ($this->accepted) {
                if ($this->onbehalf) {
                    return get_string('acceptancestatusacceptedbehalf', 'tool_iomadpolicy');
                } else {
                    return get_string('acceptancestatusaccepted', 'tool_iomadpolicy');
                }

            } else if ($this->declined) {
                if ($this->onbehalf) {
                    return get_string('acceptancestatusdeclinedbehalf', 'tool_iomadpolicy');
                } else {
                    return get_string('acceptancestatusdeclined', 'tool_iomadpolicy');
                }

            } else {
                return get_string('acceptancestatuspending', 'tool_iomadpolicy');
            }
            ...
            if (count($this->accepted) == count($this->versions)) {
                ...
                return get_string('acceptancestatuspartial', 'tool_iomadpolicy');
