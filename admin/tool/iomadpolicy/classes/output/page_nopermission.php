...
class page_nopermission implements renderable, templatable {
    ...
    /** @var bool True if user has permission to accept iomadpolicy documents; false otherwise. */
     ...
     *
     * @param array $versionids int[] List of iomadpolicy version ids that were checked.
    ...
    public function __construct(array $versionids, $behalfid) {
        ...
        $this->policies = api::list_current_versions(iomadpolicy_version::AUDIENCE_LOGGEDIN);
        ...
        if (empty($this->policies) && !empty($USER->id)) {
            ...
            if (!$currentuser->policyagreed) {
                ...
                api::update_iomadpolicyagreed($currentuser);
            }
        }
    ...
    protected function prepare_global_page_access() {
        ...
        $myurl = new moodle_url('/admin/tool/iomadpolicy/index.php', [
    ...
    public function export_for_template(renderer_base $output) {
        ...
        if (!$this->haspermissionagreedocs) {
            if (!empty($this->behalfuser)) {
                ...
                $messagetitle = get_string('nopermissiontoagreedocsbehalf', 'tool_iomadpolicy');
                $messagedesc = get_string('nopermissiontoagreedocsbehalf_desc', 'tool_iomadpolicy', $data->behalfuser);
            } else {
                $messagetitle = get_string('nopermissiontoagreedocs', 'tool_iomadpolicy');
                $messagedesc = get_string('nopermissiontoagreedocs_desc', 'tool_iomadpolicy');
            }
        }
        ...
        // Add policies list.
        $policieslinks = array();
        foreach ($this->policies as $iomadpolicyversion) {
            // Get a link to display the full iomadpolicy document.
            $iomadpolicyurl = new moodle_url('/admin/tool/iomadpolicy/view.php',
                array('iomadpolicyid' => $iomadpolicyversion->iomadpolicyid, 'returnurl' => qualified_me()));
            $iomadpolicyattributes = array('data-action' => 'view',
                                      'data-versionid' => $iomadpolicyversion->id,
                                      'data-behalfid' => $this->behalfid);
            $policieslinks[] = html_writer::link($iomadpolicyurl, $iomadpolicyversion->name, $iomadpolicyattributes);
        }
