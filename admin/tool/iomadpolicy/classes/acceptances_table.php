...
use tool_iomadpolicy\output\acceptances_filter;
use tool_iomadpolicy\output\renderer;
use tool_iomadpolicy\output\user_agreement;
use core_user;
use stdClass;
...
class acceptances_table extends \table_sql {
    ...
    public function __construct($uniqueid, acceptances_filter $acceptancesfilter, renderer $output) {
        ...
        if (!$this->is_downloading() && !has_capability('tool/iomadpolicy:acceptbehalf', \context_system::instance())) {
    ...
    protected function configure_for_single_version() {
        ...
        $join = "JOIN {tool_iomadpolicy_acceptances} a{$v} ON a{$v}.userid = u.id AND a{$v}.iomadpolicyversionid=:versionid{$v}";
        ...
        $this->add_column_header('status' . $v, get_string('response', 'tool_iomadpolicy'));
        $this->add_column_header('timemodified', get_string('responseon', 'tool_iomadpolicy'));
        $this->add_column_header('usermodified' . $v, get_string('responseby', 'tool_iomadpolicy'));
        $this->add_column_header('note', get_string('acceptancenote', 'tool_iomadpolicy'), false);
    }
    ...
    protected function configure_for_multiple_versions() {
        $this->add_column_header('statusall', get_string('acceptancestatusoverall', 'tool_iomadpolicy'));
        $filterstatus = $this->acceptancesfilter->get_status_filter();
        $statusall = [];
    ...
    protected function build_sql_for_capability_filter() {
        ...
        list($neededroles, $forbiddenroles) = get_roles_with_cap_in_context(\context_system::instance(), 'tool/iomadpolicy:accept');
    ...
    public function wrap_html_start() {
        echo \html_writer::start_tag('form',
            ['action' => new \moodle_url('/admin/tool/iomadpolicy/accept.php')]);
    ...
    public function wrap_html_finish() {
        ...
        if ($this->canagreeany) {
            ...
            echo \html_writer::empty_tag('input', ['type' => 'submit', 'data-action' => 'acceptmodal',
                'value' => get_string('consentbulk', 'tool_iomadpolicy'), 'class' => 'btn btn-primary mt-1']);
    ...
    public function format_row($row) {
        ...
        $row->select = null;
        if (!$this->is_downloading()) {
            if (has_capability('tool/iomadpolicy:acceptbehalf', \context_system::instance()) ||
                has_capability('tool/iomadpolicy:acceptbehalf', \context_user::instance($row->id))) {
                $row->canaccept = true;
                $row->select = \html_writer::empty_tag('input',
                    ['type' => 'checkbox', 'name' => 'userids[]', 'value' => $row->id, 'class' => 'usercheckbox',
                    'id' => 'selectuser' . $row->id]) .
                \html_writer::tag('label', get_string('selectuser', 'tool_iomadpolicy', $this->username($row->user, false)),
