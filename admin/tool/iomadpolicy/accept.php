...
$PAGE->set_url(new moodle_url('/admin/tool/iomadpolicy/accept.php'));
...
if ($returnurl) {
    ...
    $returnurl = new moodle_url('/admin/tool/iomadpolicy/user.php', ['userid' => $userid]);
} else {
    $returnurl = new moodle_url('/admin/tool/iomadpolicy/acceptances.php');
}
// Initialise the form, this will also validate users, versions and check permission to accept policies.
$form = new \tool_iomadpolicy\form\accept_iomadpolicy(null,
...
echo $output->header();
echo $output->heading(get_string('statusformtitle'.$action, 'tool_iomadpolicy'));
$form->display();
echo $output->footer();
