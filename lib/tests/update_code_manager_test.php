...
class update_code_manager_test extends \advanced_testcase {
    ...
    public function test_get_remote_plugin_zip() {
        ...
        $this->assertFalse($codeman->get_remote_plugin_zip('https://first/', ''));
        $this->assertDebuggingCalled('Error fetching plugin ZIP: md5 mismatch.');
        $this->assertEquals(1, $codeman->downloadscounter);
        $this->assertNotFalse($codeman->get_remote_plugin_zip('https://first/', md5('https://first/')));
        $this->assertEquals(2, $codeman->downloadscounter);
        $this->assertNotFalse($codeman->get_remote_plugin_zip('https://two/', md5('https://two/')));
    ...
    public function test_get_remote_plugin_zip_corrupted_cache() {
        ...
        file_put_contents($temproot.'/distfiles/'.md5('https://valid/').'.zip', 'https://invalid/');

        // Even if the cache file is already there, its name does not match its
        // actual content. It must be removed and re-downaloaded.
        $returned = $codeman->get_remote_plugin_zip('https://valid/', md5('https://valid/'));

        $this->assertEquals(basename($returned), md5('https://valid/').'.zip');
        $this->assertEquals(file_get_contents($returned), 'https://valid/');
    }
