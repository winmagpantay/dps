...
class moodle_url_test extends \advanced_testcase {
    ...
    public function test_moodle_url_get_path() {
        ...
        $url = new \moodle_url('https://www.example.org/');
        $this->assertSame('/', $url->get_path());

        $url = new \moodle_url('https://www.example.org/pluginfile.php/slash/arguments');
    ...
    public function test_moodle_url_round_trip() {
        $strurl = 'https://moodle.org/course/view.php?id=5';
        $url = new \moodle_url($strurl);
        $this->assertSame($strurl, $url->out(false));

        $strurl = 'https://moodle.org/user/index.php?contextid=53&sifirst=M&silast=D';
        $url = new \moodle_url($strurl);
        $this->assertSame($strurl, $url->out(false));
    }
    ...
    public function test_moodle_url_empty_param_values() {
        $strurl = 'https://moodle.org/course/view.php?id=0';
        $url = new \moodle_url($strurl, array('id' => 0));
        $this->assertSame($strurl, $url->out(false));

        $strurl = 'https://moodle.org/course/view.php?id';
    ...
    public function test_moodle_url_set_good_scheme() {
        $url = new \moodle_url('https://moodle.org/foo/bar');
    ...
    public function test_moodle_url_round_trip_array_params() {
        $strurl = 'https://example.com/?a%5B1%5D=1&a%5B2%5D=2';
        $url = new \moodle_url($strurl);
        $this->assertSame($strurl, $url->out(false));

        $url = new \moodle_url('https://example.com/?a[1]=1&a[2]=2');
        $this->assertSame($strurl, $url->out(false));

        // For un-keyed array params, we expect 0..n keys to be returned.
        $strurl = 'https://example.com/?a%5B0%5D=0&a%5B1%5D=1';
        $url = new \moodle_url('https://example.com/?a[]=0&a[]=1');
        $this->assertSame($strurl, $url->out(false));
    }
    ...
    public function test_out_as_local_url() {
        ...
        // Test https url.
        $httpswwwroot = str_replace("https://", "https://", $CFG->wwwroot);
    ...
    public function test_out_as_local_url_error() {
        $url2 = new \moodle_url('https://www.google.com/lib/tests/weblib_test.php');
        $this->expectException(\coding_exception::class);
    ...
    public function test_moodle_url_get_port() {
        ...
        $url = new \moodle_url('https://www.example.org/some/path/here.php');
