...
class curl_security_helper_test extends \advanced_testcase {
    ...
    public function curl_security_url_data_provider() {
        ...
        return [
            ...
            [$simpledns, "https://localhost/x.png", "", "", false],       // IP=127.0.0.1, Port=80 (port inferred from http).
            [$simpledns, "https://localhost:80/x.png", "", "", false],    // IP=127.0.0.1, Port=80 (specific port overrides http scheme).
            [$simpledns, "https://localhost/x.png", "", "", false],      // IP=127.0.0.1, Port=443 (port inferred from https).
            [$simpledns, "https://localhost:443/x.png", "", "", false],   // IP=127.0.0.1, Port=443 (specific port overrides http scheme).
            ...
            [$simpledns, "https://127.0.0.1/x.png", "", "", false],       // IP=127.0.0.1, Port=80 (port inferred from http).
            [$simpledns, "127.0.0.1/x.png", "", "", false],              // IP=127.0.0.1, Port=80 (port inferred from http fallback).
            [$simpledns, "https://localhost:8080/x.png", "", "", false],  // IP=127.0.0.1, Port=8080 (port hard specified).
            [$simpledns, "https://192.168.1.10/x.png", "", "", false],    // IP=192.168.1.10, Port=80 (port inferred from http).
            [$simpledns, "https://192.168.1.10/x.png", "", "", false],   // IP=192.168.1.10, Port=443 (port inferred from https).
            [$simpledns, "https://sub.example.com/x.png", "", "", false], // IP=::1, Port = 80 (port inferred from http).
            [$simpledns, "https://s-1.d-1.com/x.png", "", "", false],     // IP=::1, Port = 80 (port inferred from http).

            // Test set using domain name filters but with all ports allowed (empty).
            [$simpledns, "https://localhost/x.png", "localhost", "", true],
            ...
            [$simpledns, "https://sub.example.com/x.png", "localhost", "", false],
            [$simpledns, "https://example.com/x.png", "example.com", "", true],
            [$simpledns, "https://sub.example.com/x.png", "example.com", "", false],

            // Test set using wildcard domain name filters but with all ports allowed (empty).
            [$simpledns, "https://sub.example.com/x.png", "*.com", "", true],
            [$simpledns, "https://example.com/x.png", "*.example.com", "", false],
            [$simpledns, "https://sub.example.com/x.png", "*.example.com", "", true],
            [$simpledns, "https://sub.example.com/x.png", "*.sub.example.com", "", false],
            [$simpledns, "https://sub.example.com/x.png", "*.example", "", false],

            // Test set using IP address filters but with all ports allowed (empty).
            [$simpledns, "https://localhost/x.png", "127.0.0.1", "", true],
            [$simpledns, "https://127.0.0.1/x.png", "127.0.0.1", "", true],

            // Test set using CIDR IP range filters but with all ports allowed (empty).
            [$simpledns, "https://localhost/x.png", "127.0.0.0/24", "", true],
            [$simpledns, "https://127.0.0.1/x.png", "127.0.0.0/24", "", true],

            // Test set using last-group range filters but with all ports allowed (empty).
            [$simpledns, "https://localhost/x.png", "127.0.0.0-30", "", true],
            [$simpledns, "https://127.0.0.1/x.png", "127.0.0.0-30", "", true],

            // Test set using port filters but with all hosts allowed (empty).
            [$simpledns, "https://localhost/x.png", "", "80\n443", false],
            [$simpledns, "https://localhost:80/x.png", "", "80\n443", false],
            [$simpledns, "https://localhost/x.png", "", "80\n443", false],
            [$simpledns, "https://localhost:443/x.png", "", "80\n443", false],
            [$simpledns, "https://sub.example.com:8080/x.png", "", "80\n443", true],
            [$simpledns, "https://sub.example.com:-80/x.png", "", "80\n443", true],
            [$simpledns, "https://sub.example.com:aaa/x.png", "", "80\n443", true],

            // Test set using port filters and hosts filters.
            [$simpledns, "https://localhost/x.png", "127.0.0.1", "80\n443", true],
            [$simpledns, "https://127.0.0.1/x.png", "127.0.0.1", "80\n443", true],
            ...
            [$multiplerecorddns, "https://sub.example.com", '1.2.3.4', "", true],
            [$multiplerecorddns, "https://sub.example.com", '5.6.7.8', "", true],

            // Test when DNS resolution fails.
            [[], "https://example.com", "127.0.0.1", "", true],

            // Test ensures that the default value of getremoteaddr() 0.0.0.0 will check against the provided blocked list.
            [$simpledns, "https://0.0.0.0/x.png", "0.0.0.0", "", true],
            // Test set using IPV4 with integer format.
            [$simpledns, "https://2852039166/x.png", "169.254.169.254", "", true],

            // Test some freaky deaky Unicode domains. Should be blocked always.
            [$simpledns, "https://169。254。169。254/", "127.0.0.1", "", true],
            [$simpledns, "https://169。254。169。254/", "1.2.3.4", "", true],
            [$simpledns, "https://169。254。169。254/", "127.0.0.1", "80\n443", true]

            // Note on testing URLs using IPv6 notation:
            // At present, the curl_security_helper class doesn't support IPv6 url notation.
            // E.g.  https://[ad34::dddd]:port/resource
            ...
            // 1. ["https://[::1]/x.png", "", "", false]
            // 2. ["https://[::1]/x.png", "::1", "", true]
            // 3. ["https://[::1]/x.png", "::1/64", "", true]
            // 4. ["https://[fe80::dddd]/x.png", "fe80::cccc-eeee", "", true]
            // 5. ["https://[fe80::dddd]/x.png", "fe80::dddd/128", "", true].
        ];
    }
