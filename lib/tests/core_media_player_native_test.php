...
class core_media_player_native_test extends \advanced_testcase {
    ...
    public function test_list_supported_urls() {
        ...
        $urls = array_map(function($ext){
            return new \moodle_url('https://example.org/video.' . $ext);
        }, $nativeextensions);
    ...
    public function test_get_attribute() {
        $urls = [
            new \moodle_url('https://example.org/some_filename.mp4'),
            new \moodle_url('https://example.org/some_filename_hires.mp4'),
        ];
    ...
    public function test_replace_sources() {
        ...
        $this->assertStringContainsString('<source src="https://example.org/some_filename.mp4" />', $content);
        $this->assertStringContainsString('<source src="https://example.org/some_filename_hires.mp4" />', $content);

        // Change sources.
        $newsource = '<source src="https://example.org/new_filename.mp4" />';
        $content = media_test_native_plugin::replace_sources($content, $newsource);
        $this->assertStringContainsString($newsource, $content);
        $this->assertStringNotContainsString('<source src="https://example.org/some_filename.mp4" />', $content);
        $this->assertStringNotContainsString('<source src="https://example.org/some_filename_hires.mp4" />', $content);
    }
}
