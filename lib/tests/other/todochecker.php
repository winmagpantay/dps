...
if (empty($found)) {
    ...
    foreach ($found as $filepath => $matches) {
        ...
        foreach ($matches as $lineno => $line) {
            $url = 'https://cvs.moodle.org/moodle/' . $filepath . '?view=annotate#l' . $lineno;
            $error = '';
            ...
            if (preg_match('/\bTODO\b.*?\b(MDL-\d+)/', $line, $matches)) {
                $issueid = $matches[1];
                $issueurl = 'https://tracker.moodle.org/browse/' . $issueid;
...
function issue_info($issueid) {
    ...
    $xmlurl = 'https://tracker.moodle.org/si/jira.issueviews:issue-xml/' . $issueid . '/' . $issueid . '.xml';
