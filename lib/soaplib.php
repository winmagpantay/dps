 ...
 *             {@link https://maths.york.ac.uk/serving_maths}
 *             and others
 */
...
// Fix simple type encoding - work around a bug in early versions of PHP5 < 5.0.3, see https://bugs.php.net/bug.php?id=31832
...
// Fix complex type encoding - work around a bug in early versions of PHP5 < 5.0.3, see https://bugs.php.net/bug.php?id=31832
...
// Fix array encoding - work around a bug in early versions of PHP5 < 5.0.3, see https://bugs.php.net/bug.php?id=31832
...
function handle_soap_wsdl_request($wsdlfile, $address=false) {
    ...
    if (false !== $address) {
        if (true === $address) {
            $address = (($_SERVER['SERVER_PORT'] == 443) ? 'https://' : 'https://') . $_SERVER['SERVER_NAME'] . $_SERVER['SCRIPT_NAME'];
        }
