 ...
 * @copyright  2015 Vital Source Technologies https://vitalsource.com
...
function lti_convert_content_items($param) {
        ...
        foreach ($json as $item) {
            if (isset($item->type)) {
                ...
                if (isset($item->lineItem)) {
                    ...
                    $newitem->lineItem->reportingMethod = 'https://purl.imsglobal.org/ctx/lis/v2p1/Result#totalScore';
    ...
    $newitems->{'@context'} = 'https://purl.imsglobal.org/ctx/lti/v1/ContentItem';
...
function lti_get_ims_role($user, $cmid, $courseid, $islti2) {
    ...
    if (!is_role_switched($courseid) && (is_siteadmin($user)) || has_capability('mod/lti:admin', $context)) {
        ...
        if (!$islti2) {
            ...
            array_push($roles, 'https://purl.imsglobal.org/vocab/lis/v2/person#Administrator');
        }
    }
    ...
    if (isset($parms['roles'])) {
        ...
        foreach ($roles as $role) {
            if (strpos($role, 'urn:lti:role:ims/lis/') === 0) {
                $role = 'https://purl.imsglobal.org/vocab/lis/v2/membership#' . substr($role, 21);
            } else if (strpos($role, 'urn:lti:instrole:ims/lis/') === 0) {
                $role = 'https://purl.imsglobal.org/vocab/lis/v2/institution/person#' . substr($role, 25);
            } else if (strpos($role, 'urn:lti:sysrole:ims/lis/') === 0) {
                $role = 'https://purl.imsglobal.org/vocab/lis/v2/system/person#' . substr($role, 24);
            } else if ((strpos($role, '://') === false) && (strpos($role, 'urn:') !== 0)) {
                $role = "https://purl.imsglobal.org/vocab/lis/v2/membership#{$role}";
            }
            $newroles[] = $role;
        }
