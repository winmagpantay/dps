...
class memberships extends \mod_lti\local\ltiservice\service_base {
    ...
    const CONTEXT_ROLE_PREFIX = 'https://purl.imsglobal.org/vocab/lis/v2/membership#';
    /** Context-level role for Instructor */
    const CONTEXT_ROLE_INSTRUCTOR = 'https://purl.imsglobal.org/vocab/lis/v2/membership#Instructor';
    /** Context-level role for Learner */
    const CONTEXT_ROLE_LEARNER = 'https://purl.imsglobal.org/vocab/lis/v2/membership#Learner';
    ...
    public function get_members_json($resource, $context, $course, $role, $limitfrom, $limitnum, $lti, $info, $response) {
        ...
        $exclude = array();
        if (!empty($role)) {
            if ((strpos($role, 'https://') !== 0) && (strpos($role, 'https://') !== 0)) {
    ...
    private function users_to_jsonld($resource, $users, $contextid, $exclude, $limitfrom, $limitnum,
        ...
        $arrusers = [
            '@context' => 'https://purl.imsglobal.org/ctx/lis/v2/MembershipContainer',
            '@type' => 'Page',
